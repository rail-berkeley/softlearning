...
class SwimmerEnv(MujocoEnv, Serializable):
    ...
    def step(self, action):
        ...
        if self.undirected:
            forward_reward = abs(forward_reward)
    ...
    def log_diagnostics(self, paths):
        if len(paths) > 0:
            ...
            if self.undirected:
                n_directions = [
                    np.max(progs) > self.progress_threshold,
                    np.min(progs) < - self.progress_threshold,
                    ].count(True)
                logger.record_tabular('nDirections', n_directions)
        else:
