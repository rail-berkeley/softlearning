    ...
    def __init__(self, env, minimum, maximum):
        ...
        if not (np.all(np.isfinite(action_spec.minimum)) and
            ...
            raise ValueError(_BOUNDS_MUST_BE_FINITE.format(
                object_type='env.action_spec()', action_spec=action_spec))
        ...
        if not np.all(np.isfinite(minimum)):
            raise ValueError(_BOUNDS_MUST_BE_FINITE.format(
                object_type='minimum', action_spec=minimum))

        assert minimum.shape == action_spec.minimum.shape, (
            _MUST_BE_BROADCASTABLE.format(
                bound_type="minimum",
        ...
        if not np.all(np.isfinite(maximum)):
            raise ValueError(_BOUNDS_MUST_BE_FINITE.format(
                object_type='maximum', action_spec=maximum))
