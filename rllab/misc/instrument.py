...
def run_experiment_lite(
    ...
    the end of the experiment)
    :param sync_s3_h5: Whether to sync h5 files during execution of the experiment (they will always be synced at
    the end of the experiment)
...
def launch_ec2(params_list, exp_prefix, docker_image, code_full_path,
    ...
    for params in params_list:
        ...
        if periodic_sync:
            ...
            include_h5 = " --include '*.h5' " if sync_s3_h5 else " "
            # sio.write("""
            #     while /bin/true; do
            ...
            sio.write("""
                while /bin/true; do
                    aws s3 sync --exclude '*' {include_png} {include_pkl} {include_log} {include_html} {include_h5}--include '*.csv' --include '*.json' {log_dir} {remote_log_dir}
                    sleep {periodic_sync_interval}
                done & echo sync initiated""".format(include_png=include_png, include_pkl=include_pkl,
                                                     include_log=include_log, include_html=include_html, include_h5=include_h5,
                                                     log_dir=log_dir, remote_log_dir=remote_log_dir,
                                                     periodic_sync_interval=periodic_sync_interval))
    ...
    # send a termination report (need aws credentials)
    if hasattr(config, "SNS_TOPIC_ARN"):
        exp_names = ""
        for params in params_list:
            exp_names += params["exp_name"]
            exp_names += ", "
        sio.write("""
            export AWS_ACCESS_KEY_ID={aws_access_key}
            export AWS_SECRET_ACCESS_KEY={aws_access_secret}
            export AWS_DEFAULT_REGION={sns_topic_arn_region}
            aws sns publish --topic-arn {arn} --message \"{msg}\"
        """.format(
            aws_access_key=config.AWS_ACCESS_KEY,
            aws_access_secret=config.AWS_ACCESS_SECRET,
            sns_topic_arn_region=config.SNS_TOPIC_ARN_REGION,
            arn=config.SNS_TOPIC_ARN,
            msg="The following experiments are terminated: %s"%(exp_names)
        ))
